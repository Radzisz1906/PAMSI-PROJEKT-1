...
Funkcja ma na celu zmusic quick_sort do uzywania elementu œrodkowego jako pivot, zamiast elementu ostatniego.
Poprawia to wydajnoœæ w niektórych przypadkach*/
...
void merge ( long int tab[],long int pierwszy_elem, long int ostatni_elem, long int srodek)
{
 ...
 // GLOWNY ALGORYTM SORTUJACY I £¥CZACY DWIE POLOWY
    ...
    // SORTOWANIE I £ACZENIE ELEMENTÓW KTÓRE POMIELIŒMY
    while (it_PP < PP )
    {
        tab[it_pocz] = tempP[it_PP];
       it_PP++;
...
void mergesort(long int tab[],long int pierwszy_elem, long int ostatni_elem)
{
    if (pierwszy_elem < ostatni_elem )  // WYKONYWANIE DOPOKI NIE DOSZLISMY DO OSTATNIEGO ELEMENTU
    {
       long int  srodek = pierwszy_elem + (ostatni_elem - pierwszy_elem)/2;  // WYBIERANIE ELEMENTU SRODKOWEGO
        ...
        merge(tab,pierwszy_elem,ostatni_elem,srodek); // £¥CZENIE ELEMENTÓW W POSORTOWANY CI¥G
    }
}
/*********************************************************************************************/

/*********************************FUNKCJE POMOCNICZE*************************************************/
/*wyp_full_los - tworzenie dowolnie du¿ej tablicy nieposortowanych liczb*/
/*posortowana_odwrotnie - tworzenie tablicy liczb posortowanych w odwrotnej kolejnoœci*/
/*wypelniona_procent - tworzenie tablicy liczb, która ju¿ czêœciowo jest posortowana*/
